...
class FileSystem:
    ...
    def remove(self, path):
        ...
        if path_parts[0] == '.':
            ...
            current_directory = self._root

        for part in path_parts[:-1]:
            current_directory = self._searchHelper(current_directory, part)

            if current_directory is None:
                print(f"Directory '{part}' not found.")
                return

        file_or_dir_name = path_parts[-1]
        target_file_or_dir = self._searchHelper(current_directory, file_or_dir_name)

        if target_file_or_dir is None:
            print(f"File or directory '{file_or_dir_name}' not found.")
            return

        if isinstance(target_file_or_dir, Directory):
            target_file_or_dir.rmr(current_directory)
        else:
            target_file_or_dir.rm(current_directory)
